...
### <a name="define-the-class-you-want-to-expose-to-other-solutions"></a>다른 솔루션에 노출 하려는 클래스를 정의 합니다.  
 최소한, 노출 하려는 클래스를 public 이어야, 있어야 합니다 <xref:System.Runtime.InteropServices.ComVisibleAttribute> 특성이로 설정 **true**, 노출 해야 합니다는 [IDispatch](/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch) 인터페이스.  
  
 노출 하는 방법이 권장된 합니다 [IDispatch](/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch) 인터페이스는 다음 단계를 수행 하려면:  
