...
- 외부 데이터를 Visio로 가져오고, 셰이프에 연결하고, 페이지에 그래픽으로 표시합니다.  
  ...
  단계별 절차를 볼 수 있으며 Visio의 개체 모델을 사용 하 여 문서 및 셰이프를 사용 하는 예제 코드 [Visio 문서 작업](../vsto/working-with-visio-documents.md) 하 고 [Visio 셰이프 작업](../vsto/working-with-visio-shapes.md)합니다.  
  
  VSTO 추가 기능에서 Visio 개체 모델에 액세스하려면 프로젝트에서 `Application` 클래스의 `ThisAddIn` 필드를 사용합니다. `Application` 필드는 Visio의 현재 인스턴스를 나타내는 `Microsoft.Office.Interop.Visio.Application` 개체를 반환합니다. 자세한 내용은 [프로그램 VSTO 추가 기능](../vsto/programming-vsto-add-ins.md)합니다.  
  
  Visio 개체 모델을 호출할 때 Visio용 PIA(주 Interop 어셈블리)에서 제공되는 형식을 사용합니다. PIA는 VSTO 추가 기능의 관리 코드와 Visio의 COM 개체 모델 간의 다리 역할을 합니다. Visio PIA의 모든 형식은 `Microsoft.Office.Interop.Visio` 네임스페이스에 정의되어 있습니다. 주 interop 어셈블리에 대 한 자세한 내용은 참조 하세요. [Office 솔루션 개발 개요 &#40;VSTO&#41; ](../vsto/office-solutions-development-overview-vsto.md) 하 고 [Office 주 interop 어셈블리](../vsto/office-primary-interop-assemblies.md).  
  
## <a name="visio-object-model-overview"></a>Visio 개체 모델 개요  
 Visio 개체 모델의 개요를 확인할 수 있습니다 [Visio 개체 모델 개요](../vsto/visio-object-model-overview.md), Visio 개체 모델 참조 및 Sdk에 대 한 링크를 포함 합니다.  
  
## <a name="customize-the-user-interface-of-visio"></a>Visio의 사용자 인터페이스  
