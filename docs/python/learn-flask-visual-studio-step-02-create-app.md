...
1. `HelloFlask` 폴더에서 다음 콘텐츠가 포함된 `views.py`라는 파일을 만듭니다. `__init__.py` 내에서 `import HelloFlask.views`를 사용했기 때문에 `views.py` 이름은 중요합니다. 이름이 일치하지 않으면 런타임에 오류가 표시됩니다.
    ...
    이 코드는 함수의 이름과 경로를 `home`으로 변경하는 것 외에도, app.py의 페이지 렌더링 코드를 포함하고 `__init__.py`에 선언된 `app` 개체를 가져옵니다.
...
1. 또한 코드의 여러 부분에서 중단점을 설정하고 앱을 다시 시작하여 시작 시퀀스를 따를 수 있습니다. 예를 들어 `runserver.py` 및 `HelloFlask\__init__.py`의 첫 번째 줄과 `views.py`의 `return "Hello Flask!"` 줄에 중단점을 설정합니다. 그런 다음, 앱을 다시 시작(**디버그** > **다시 시작**, Ctrl + F5 또는 아래 표시된 도구 모음 단추)하고 코드를 단계별로 실행(F10)하거나 F5 키를 사용하여 각 중단점에서 실행합니다.
...
### <a name="question-how-frequently-should-one-commit-to-source-control"></a>질문: 소스 제어에 얼마나 자주 커밋해야 합니까?
...
### <a name="question-can-templates-be-organized-into-further-subfolders"></a>질문: 템플릿을 추가 하위 폴더로 구성할 수 있습니까?
