...
### <a name="question-how-do-i-handle-url-variables-and-query-parameters-in-an-api"></a>질문: API에서 URL 변수와 쿼리 매개 변수를 어떻게 처리합니까?

답변: [질문: Flask는 다양한 URL 경로와 쿼리 매개 변수에서 어떻게 작동합니까?](learn-flask-visual-studio-step-01-project-solution.md#qa-url-variables)에 대한 1-4단계의 답변을 참조하세요.
...
1. **솔루션 탐색기**에서 `templates` 폴더를 마우스 오른쪽 단추로 클릭하고 **추가** > **새 항목**을 선택한 후 “HTML 페이지” 항목 템플릿을 선택하고 파일의 이름을 `about.html`로 지정한 다음, **확인**을 선택합니다.
...
1. `templates/about.html`도 기본 템플릿을 참조하고 콘텐츠 블록을 재정의하도록 수정합니다.
