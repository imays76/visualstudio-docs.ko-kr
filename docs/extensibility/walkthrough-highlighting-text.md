...
5.  상속 되는 두 번째 클래스를 만듭니다 <xref:Microsoft.VisualStudio.Text.Classification.MarkerFormatDefinition>, 하 고 이름을 `HighlightWordFormatDefinition`입니다. 이 형식 정의 태그를 사용 하려면 다음과 같은 특성을 사용 하 여 내보내기 해야 합니다.  
...
7.  HighlightWordTag에 대 한 생성자에서 만든 형식 정의 이름을 전달 합니다.  
