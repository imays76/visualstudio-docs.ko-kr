...
1.  에 설명 된 대로 시작 페이지 프로젝트를 만듭니다 [사용자 지정 시작 페이지를 만들려면](creating-a-custom-start-page.md)합니다. 프로젝트 이름을 **SaveMySettings**합니다.  
...
5.  디자인 창에서 컨트롤의 주요 영역을 클릭 한 다음 **삭제**합니다.  
  
     이 단계를 제거 합니다 <xref:System.Windows.Controls.Border> 요소 및 그 안에 모든 커지고 상위 항목만 수준 <xref:System.Windows.Controls.Grid> 요소입니다.  
...
1.  XAML 창에서 마우스 오른쪽 단추로 클릭 합니다 `Click` 특성을 <xref:System.Windows.Controls.Button> 요소와 클릭 한 다음 **이벤트 처리기로 이동**.  
     ...
     이 단계를 엽니다 *MyControl.xaml.cs*에 대 한 스텁 처리기를 만들고 여 `Button_Click` 이벤트입니다.  
...
5.  에 대 한 다음 처리기를 추가 합니다 `OnLoaded` 사용자 컨트롤의 이벤트입니다.  
    ...
    ```  
     ...
     이 코드는 입력란의 텍스트 "MySetting"의 현재 값을 설정합니다.  
  
6.  사용자 정의 컨트롤을 빌드하십시오.  
  
7.  **솔루션 탐색기**오픈 *source.extension.vsixmanifest*합니다.  
  
8.  매니페스트 편집기에서 설정 **Product Name** 하 **내 설정 시작 페이지 저장**합니다.  
  
     에 표시 하는 것 만큼 시작 페이지의 이름을 설정 하는이 기능을 **시작 페이지 사용자 지정** 목록에 **옵션** 대화 상자.  
  
9. 빌드할 *StartPage.xaml*합니다.  
...
9. 시작 페이지를 닫고 다시 여십시오.  
     ...
     Visual Studio 자체를 닫을 때까지 닫혀 있는 경우에 Visual Studio 도구 창 메모리에 유지 하기 때문에 설정을 저장 하지 않은 하는 경우에 word "Dog" 텍스트 상자에 표시 합니다.  
  
10. Visual Studio의 실험적 인스턴스를 닫습니다.  
  
11. 키를 눌러 **F5** 를 실험적 인스턴스를 다시 엽니다.  
  
12. 텍스트 상자에 "Cat" 단어를 표시 합니다.  
  
## <a name="next-steps"></a>다음 단계  
 저장 하 고 가져오기 및 설정에 다른 이벤트 처리기에서 다른 값을 사용 하 여 개수에 관계 없이 사용자 지정 설정 검색할이 사용자 정의 컨트롤을 수정할 수는 `SettingsStore` 속성입니다. 사용 하는 다른 `propertyName` 매개 변수를 호출할 때마다 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore.SetString%2A>, 값을 레지스트리에서 서로 덮어쓰지 마십시오.  
