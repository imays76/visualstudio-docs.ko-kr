...
-   **UI 컨텍스트** UI 컨텍스트 변수의 값으로 식별 됩니다는 <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT> 클래스, 예를 들어 <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT.SolutionBuilding_guid> 솔루션을 작성할 때 또는 <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT.Debugging_guid> 디버거가 활성화 되어 있습니다. 여러 UI 컨텍스트는 동시에 활성화할 수 있습니다.
