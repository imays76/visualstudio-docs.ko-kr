...
|*\<GUID &GT;*|REG_SZ|""|논리 뷰 지원에 대 한 키입니다. 필요에 따라 이러한 만큼 가질 수 있습니다. 레지스트리 항목의 이름 것은 중요 한 값이 아닌은 항상 빈 문자열입니다.|  
