호스팅 서버에 대 한 웹 배포 3.6 UI에서 게시 설정 파일을 만들 수 있는 추가 구성 기능을 제공 합니다.

1. 웹 배포 3.6 Windows 서버에 이미 설치 된 경우에 사용 하 여 제거 **Control Panel** > **프로그램** > **프로그램제거**.

1. 다음으로, Windows server 호스팅 서버에 대 한 웹 배포 3.6을 설치 합니다.
...
1. (선택 사항) 열어 웹 배포 제대로 실행 되는지 확인 **제어판 > 시스템 및 보안 > 관리 도구 > 서비스** 했는지 **웹 배포 에이전트 서비스가** 중인지 ( 서비스 이름은 이전 버전에서 다른)입니다.
