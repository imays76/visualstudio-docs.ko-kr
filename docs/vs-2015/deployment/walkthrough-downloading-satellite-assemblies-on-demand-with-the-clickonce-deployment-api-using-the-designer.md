...
이 항목의 최신 버전에서 찾을 수 있습니다 [연습: ClickOnce 배포 API를 사용 하 여 디자이너는 요청 시 위성 어셈블리 다운로드](https://docs.microsoft.com/visualstudio/deployment/walkthrough-downloading-satellite-assemblies-on-demand-with-the-clickonce-deployment-api-using-the-designer)합니다.  
...
3.  클릭 합니다 **게시** 탭을 클릭 한 다음 **응용 프로그램 파일**합니다.  
  
4.  선택 된 **모든 파일 표시** 위성 어셈블리를 표시 하려면 확인란 합니다. 기본적으로 모든 위성 어셈블리는 배포에 포함되고 이 대화 상자에 표시됩니다.  
  
     위성 어셈블리 형식에서 이름을 가진 *isoCode*\ApplicationName.resources.dll, 여기서 *isoCode* 는 RFC 1766 형식의 언어 식별자입니다.  
  
5.  클릭 **새로 만들기...**  에 **다운로드 그룹** 각 언어 식별자에 대 한 목록입니다. 다운로드 그룹 이름을 요청하는 메시지가 표시되면 언어 식별자를 입력합니다. 예를 들어 일본어 위성 어셈블리에 대 한 지정 다운로드 그룹 이름을 `ja-JP`입니다.  
  
6.  닫기 합니다 **응용 프로그램 파일** 대화 상자.  
  
### <a name="to-download-satellite-assemblies-on-demand-in-c"></a>C#에서 요청 시 위성 어셈블리를 다운로드하려면  
  
1.  Program.cs 파일을 엽니다. 솔루션 탐색기에서 파일을 프로젝트에 선택이 표시 되지 않으면 및 합니다 **프로젝트** 메뉴에서 클릭 **모든 파일 표시**합니다.  
...
1.  에 **속성** 응용 프로그램에 대 한 창을 클릭 합니다 **응용 프로그램** 탭 합니다.  
  
2.  탭 페이지의 맨 아래에서 클릭 **응용 프로그램 이벤트 보기**합니다.  
