...
6.  업데이트를 확인 하려면 사용자를 선택 하는 Windows 폼에 단추, 메뉴 항목 또는 기타 사용자 인터페이스 항목을 만듭니다. 해당 항목의 이벤트 처리기에서 확인 하 고 업데이트를 설치 하는 메서드를 호출 합니다. 이러한 메서드에 대 한 Visual Basic 및 Visual C# 코드 예제를 찾을 수 있습니다 [방법: ClickOnce 배포 API 응용 프로그램 업데이트 프로그래밍 방식으로 사용 하는 것에 대 한 확인](../deployment/how-to-check-for-application-updates-programmatically-using-the-clickonce-deployment-api.md)합니다.  
