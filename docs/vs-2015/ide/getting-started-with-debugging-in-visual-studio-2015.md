...
### <a name="performing-static-code-analysis"></a>정적 코드 분석 수행  
 ...
 ![경고를 사용 하 여 visual Studio 2015 오류 목록](../ide/media/vs-ide-gs-debug-code-analysis-warning-error-list.PNG "vs_ide_gs_debug_code_analysis_warning_error_list")  
...
### <a name="debugging-your-running-code"></a>실행 중인 코드 디버그  
 성공적으로 코드를 빌드하고 약간의 정리 작업을 수행했으므로 이제 F5 키를 누르거나 **디버그->디버깅 시작**을 선택하여 실행합니다. 그러면 디버그 환경에서 앱이 시작되므로 해당 동작을 자세히 관찰할 수 있습니다. Visual Studio 2015 IDE는 앱이 실행 되는 동안 변경: 합니다 **출력** 창 (기본 창 구성)에서 새로운 두 바뀝니다 합니다 **자동/지역/모듈/조사식** 탭된 창 및 **호출 스택/중단점/예외 설정/출력** 탭된 창입니다. 이러한 창에는 실행 시 앱의 변수, 스레드, 호출 스택 및 기타 다양한 동작을 검사 및 평가할 수 있게 해주는 여러 탭이 있습니다.  
  
 ![VS2015 자동 및 호출 스택 Windows](../ide/media/vs-ide-gs-debug-autos-and-call-stack.PNG "Vs_ide_gs_debug_autos_and_call_stack")  
...
### <a name="setting-conditional-breakpoints"></a>조건부 중단점 설정  
 ...
 조건부 중단점 평가에 사용 되는 식을 선언 하는 방법에 자세한 내용을 보려면 Channel9 동영상 확인 [Visual Studio 2015의 중단점 구성 환경](http://channel9.msdn.com/Events/Visual-Studio/Connect-event-2014/711)합니다.  
...
### <a name="inspecting-your-code-at-run-time"></a>런타임에 코드 검사  
 ...
 ![실행 하는 visual Studio 2015&#45;값 검사 시간](../ide/media/vs-ide-gs-debug-inspect-value.PNG "vs_ide_gs_debug_inspect_value")  
...
### <a name="running-unit-tests"></a>단위 테스트 실행  
 ...
 Visual Studio 2015 및 어떻게 하면 더 나은 품질의 코드를 만들 수 있습니다 이러한 단위 테스트에 대 한 자세한 내용은 [단위 테스트 기본 사항](../test/unit-test-basics.md)합니다.  
