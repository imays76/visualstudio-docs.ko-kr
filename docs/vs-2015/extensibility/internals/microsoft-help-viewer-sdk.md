...
- 대체 (fallback) 브랜드 패키지 (Branding_\<로캘 >.mshc) 도움말 뷰어 2.1 응용 프로그램 루트에 설치 됩니다 (예: C:\Program Files (x86) \Microsoft Help Viewer\v2.1) 도움말 뷰어 언어 팩을 통해.  이 패키지를 브랜딩 제품 설치 되지 않은 경우에 사용 됩니다 (콘텐츠 없음 설치) 설치 된 브랜드 패키지가 없거나 손상 되었습니다.  Visual Studio 요소 (예: 로고 및 피드백) 앱 루트 대체 (fallback) 브랜드 패키지를 사용 하는 경우 무시 됩니다.
