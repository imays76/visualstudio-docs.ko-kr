...
11. 프로젝트를 빌드하고 실행합니다. 실험적 인스턴스에서 C# 유니버설 허브 앱 만들기로 이동 합니다 **도구** 메뉴를 클릭 **TestUniversalProject 호출**, 일반 출력 창에 텍스트를 확인 합니다. 공유에 있는 첫 번째 항목의 이름을 프로젝트 (예정 App.xaml 파일 수)를 변경 해야 하며 것을 확인할 수는 <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> 이벤트가 발생 한 합니다. 이 경우 App.xaml 이름을 바꾸면도 바꿀 App.xaml.cs에 있으므로 (각 플랫폼 프로젝트에 대 한 2 개) 4 개 이벤트 표시 됩니다. (DTE 이벤트 공유 프로젝트에서 항목을 추적 하지 않습니다.) 두 개의 <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A> 이벤트 (하나씩 각 플랫폼 프로젝트에 대해) 이지만 <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A> 이벤트입니다.  
