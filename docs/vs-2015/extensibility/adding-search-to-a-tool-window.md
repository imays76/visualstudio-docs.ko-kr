...
1.  에 `TestToolWindowSearch` 프로젝트, TestSearchControl.xaml 파일을 엽니다.  
...
6.  메뉴 모음에서 선택 **뷰**, **기타 Windows**를 **TestSearch**합니다.  
  
     도구 창이 표시 되지만 검색 컨트롤은 아직 표시 하지 않습니다.  
  
## <a name="to-add-a-search-box-to-the-tool-window"></a>도구 창에 검색 상자를 추가 하려면  
  
1.  TestSearch.cs 파일에 다음 코드를 추가 합니다 `TestSearch` 클래스입니다. 코드를 재정의 합니다 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWindowSearch.SearchEnabled%2A> 속성을 get 접근자를 반환 하도록 `true`합니다.  
...
1.  TestSearch.cs 파일에 다음 코드를 추가 합니다 `TestSearch` 클래스입니다. 이 코드 주문형 검색 (즉 사용자 enter 키를 눌러 필요가 없습니다) 하는 대신 빠른 검색을 활성화 합니다. 코드를 재정의 합니다 `ProvideSearchSettings` 에서 메서드는 `TestSearch` 기본 설정을 변경 하는 데 필요한 클래스입니다.  
...
5.  솔루션을 다시 빌드하면 debugb를 시작 하 여 새 설정을 테스트 합니다.  
...
1.  TestSearch.cs 파일에 다음 코드를 추가 합니다 `TestSearch` 클래스입니다. 코드를 재정의 합니다 `SearchOptionsEnum` 메서드를 지정된 된 옵션이 설정 되었는지 아니면 해제 여부를 검색할 검색 구현을 허용 합니다. 코드 `SearchOptionsEnum` 사례를 일치 하는 옵션이 추가 <xref:Microsoft.VisualStudio.Shell.Interop.IVsEnumWindowSearchOptions> 열거자입니다. 대/소문자를 일치 하는 옵션은 또한으로 사용할 수는 `MatchCaseOption` 속성입니다.  
...
2.  에 `TestSearchTask` 클래스에서 matchCase 줄 주석 처리를 제거는 `OnStartSearch` 메서드:  
...
1.  TestSearch.cs 파일에 다음 코드를 추가 합니다 `TestSearch` 클래스입니다. 코드를 구현 `SearchFiltersEnum` 더하여는 <xref:Microsoft.VisualStudio.PlatformUI.WindowSearchSimpleFilter> 지정도 줄만 표시 되도록 검색 결과 필터링 하는 합니다.  
...
2.  TestSearch.cs 파일에 다음 메서드를 추가 합니다 `TestSearchTask` 클래스에는 `TestSearch` 클래스입니다. 이러한 메서드를 지원 합니다 `OnStartSearch` 메서드를 다음 단계에서 수정할 수 있습니다.  
