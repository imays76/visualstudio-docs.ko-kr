...
> [!WARNING]
>  옵션을 선택 했는지 확인 **내 코드만 사용 (관리 전용)** 에서 합니다 **도구**에 **옵션**를 **디버깅** 하기 전에 메뉴 디버그 합니다. 두 시퀀스가 다른 시퀀스 내에 중첩 된 있고 첫 번째 내부 시퀀스에 중단점을 설정한 경우 키를 눌러 **F11** 하는 경우 두 번째 내부 시퀀스에 디버깅 되지 것입니다는 **내 코드만 사용 (관리 전용)** 을 선택 합니다.  
