...
- 식이 계산 되지 않은 • 오류가 발생 합니다. 예를 들어 시간 초과가 발생했거나 변수가 범위를 벗어났습니다.  
  
- • 식 응용 프로그램에서 부작용을 트리거할 수 있는 함수 호출을 포함 합니다 (참조 [부작용 및 식](#bkmk_sideEffects)).  
...
## <a name="dynamicview-and-the-watch-window"></a>DynamicView 및 조사식 창  
 ...
 조사식 창에서 구현 하는 형식에서 생성 하는 a 또는 개체를 표시 하는 경우는 <xref:System.Dynamic.IDynamicMetaObjectProvider>, 디버거 추가 특수 **동적 뷰** 노드를를 **자동** 표시 합니다. 이 노드에서는 동적 개체의 동적 멤버가 표시되지만 멤버의 값을 편집할 수 없습니다.  
