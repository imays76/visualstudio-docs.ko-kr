...
ms.locfileid: "37057163"
---
# <a name="set-a-watch-on-variables-using-the-watch-and-quickwatch-windows-in-visual-studio"></a>조사식 및 간략 한 조사식 Windows에서 Visual Studio를 사용 하 여 변수에 대 한 조사식 설정
을 디버깅 하는 동안 사용할 수 있습니다 합니다 **조사식** 하 고 **간략 한 조사식** 변수 및 식을 조사 하려면 windows.  차이점은 **조사식** 창에는 여러 변수가 표시될 수 있는 반면 **간략한 조사식** 창에는 한 번에 하나의 변수가 표시된다는 것입니다. 

Windows는 디버깅 세션 중만 사용할 수 있습니다. 열려는 합니다 **조사식** 창에서 선택 **디버그 > Windows > 조사식 > 조사식 (1, 2, 3, 4)**). 열려는 합니다 **간략 한 조사식** 창 하거나 변수를 마우스 오른쪽 단추로 클릭 하 고 선택 **간략 한 조사식** 선택 하거나 **디버그 > 간략 한 조사식**합니다.
...
```  
 ...
 다음과 같이 간략 한 조사식 창에서 변수를 확인할 수 있습니다.  
  
1.  `a = a + b;` 줄에 중단점을 설정합니다.  
  
2.  디버깅을 시작합니다. 중단점에서 실행이 중지됩니다.  
  
3.  열기를 **간략 한 조사식** 창 (마우스 오른쪽 단추로 클릭 `a`를 선택한 **간략 한 조사식**를 선택 하거나 `a` 키를 누릅니다 **shift+f9**).

    변수를 표시 합니다 **값** 창에 값이 1입니다.

    ![조사식](../debugger/media/watchexpression.png "QuickWatchExpression")  

    변수를 사용 하 여 식을 계산 하려면 식을 추가 같은 `a + b` 에 **식** 창과 클릭 **재평가**합니다. 
  
4.  변수를 추가 합니다 **조사식** 창에서 **간략 한 조사식** 를 클릭 하 여 **조사식 추가**합니다. 
...
5.  디버깅을 계속 (키를 눌러 **F11** 디버거 이동).  
  
 `for` 루프를 반복하면 변수 값이 변경됩니다.  
  
 네이티브 코드로 프로그래밍할 때 변수 이름 또는 변수 이름이 들어 있는 식의 컨텍스트를 한정해야 하는 경우가 있습니다. 컨텍스트는 변수가 있는 함수, 소스 파일 및 모듈입니다. 가 컨텍스트를 한 정해야 하는 경우에 컨텍스트 연산자 구문을 사용할 수 있습니다. 자세한 내용은 [Context Operator (C++)](../debugger/context-operator-cpp.md)을 참조하세요.  
...
##  <a name="bkmk_refreshWatch"></a> 만료된 조사식 값 새로 고침  
 특정 상황에서는 새로 고침 아이콘 (원형 화살표)를 확인할 수 있습니다에서 식을 계산할 때 합니다 **조사식** 창입니다.  예를 들어 속성 확인이 꺼져 있으면 (**도구 > 옵션 > 디버깅 > 속성 확인 및 기타 암시적 함수 호출**), 다음 코드를 만들었고:  
...
```  
 ...
 앞의 그림에서는 오류 또는 오래 된 값을 보여 줍니다. 일반적으로 아이콘을 클릭하면 값을 새로 고칠 수 있지만 새로 고치지 않으려는 경우도 있습니다. 먼저 값이 계산되지 않은 이유를 알아야 합니다.  
...
-   응용 프로그램에서 부작용을 트리거할 수 있는 함수 호출을 포함 하는 식 (참조 [부작용 및 식](#bkmk_sideEffects)).  
  
-   자동 속성 평가 및 암시적 함수 호출 디버거에서 해제 되어 (**도구 > 옵션 > 디버깅 > 속성 확인 및 기타 암시적 함수 호출**), 다음 식은 수 없습니다 자동으로 평가 합니다.  
  
 값을 새로 고치려면 새로 고침 아이콘을 클릭하거나 스페이스바를 누릅니다. 디버거가 식을 다시 계산합니다. 새로 고침 아이콘이 나타나면 자동 속성 평가 및 기타 암시적 함수 호출 해제 되었기 때문에 식을 계산할 수 있습니다.  
...
```  
 ...
 의도 하지 않은 것으로 알려져 있는 식은 처음 입력할 때이 한 번만 평가 됩니다. 다음 계산은 수행되지 않습니다. 값 옆에 표시되는 업데이트 아이콘을 클릭하면 이 동작을 직접 재정의할 수 있습니다.  
...
3.  표시는 **$** 더하기 숫자가 합니다 **지역** 창 개체 ID를 나타냅니다.  
...
> [!NOTE]
>  개체의 속성을 표시 하려는 경우 `Person.Name` 위의 예에서 해야 활성화 속성을 확인 합니다.  
...
## <a name="dynamic-view-and-the-watch-window"></a>동적 뷰 및 조사식 창  
 ...
 조사식 창에서 구현 하는 형식에서 만들어진 개체를 표시 하는 경우는 [IDynamicMetaObjectProvider Interface](/dotnet/api/system.dynamic.idynamicmetaobjectprovider?view=netframework-4.7), 디버거 추가 특수 **동적 뷰** 노드를는 **자동**  표시 합니다. 이 노드에서는 동적 개체의 동적 멤버가 표시되지만 멤버의 값을 편집할 수 없습니다.  
