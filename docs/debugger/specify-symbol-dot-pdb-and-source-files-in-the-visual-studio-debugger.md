...
설정 된 *web.config* 디버그 모드로 ASP.NET 응용 프로그램의 파일입니다. 디버그 모드에서는 동적으로 생성된 파일에 대한 기호가 ASP.NET에서 생성되며 디버거가 ASP.NET 애플리케이션에 연결될 수 있습니다. Visual Studio 설정이 자동으로 웹 프로젝트 템플릿에서 프로젝트를 만든 경우 디버깅을 시작 합니다.  
