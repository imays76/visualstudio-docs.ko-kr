...
ms.locfileid: "35675143"
---
# <a name="view-threads-in-the-debugger-in-visual-studio-using-the-threads-window"></a>스레드 창을 사용 하 여 Visual Studio의 디버거에서 스레드 보기
에 **스레드** 창을 검토 하 고 수 디버깅 중인 응용 프로그램에서 스레드를 사용 하 여 작동 합니다. 사용 하는 방법에 대 한 단계별 지침에 대 한 합니다 **스레드** 창 참조 [연습: 스레드 창을 사용 하 여 디버그](../debugger/how-to-use-the-threads-window.md)합니다.
...
-   현재 스레드에는 노란색 화살표는 현재 스레드 (화살표 개요-현재 스레드에 대 한 현재 디버거 컨텍스트를 나타냄)를 나타냅니다 열.
  
-   합니다 **ID** 각 스레드에 대 한 id 번호를 포함 하는 열입니다.  
  
-   합니다 **관리 되는 ID** 관리 되는 스레드의 관리 되는 id 번호를 포함 하는 열입니다.  
  
-   합니다 **범주** 사용자 인터페이스 스레드, 원격 프로시저 호출 처리기 또는 작업자 스레드로 스레드가 분류 열입니다. 특정 범주는 응용 프로그램의 주 스레드를 식별합니다.  
  
-   합니다 **이름을** 열에 있는 경우 또는 이름으로 각 스레드를 식별 하는 \<이름 없음 >.  
  
-   합니다 **위치** 열-스레드가 실행 되 고 있는 표시 됩니다. 이 위치를 확장하여 스레드의 전체 호출 스택을 표시할 수 있습니다.  
  
-   합니다 **우선 순위** 시스템에서 각 스레드에 할당 하는 우선 순위가 포함 된 열입니다.  
  
-   합니다 **Affinity Mask** 열 (일반적으로 숨김) 고급 열입니다. 이 열에는 각 스레드에 대한 프로세서 선호도 마스크가 표시됩니다. 다중 프로세서 시스템에서는 선호도 마스크에 따라 스레드가 실행될 수 있는 프로세서가 결정됩니다.  
  
-   합니다 **일시 중단 횟수** 일시 중단된 횟수를 포함 하며 일반적으로 숨겨지는 열 (일반적으로 숨김). 이 횟수에 따라 스레드를 실행할 수 있는지 여부가 결정됩니다. 일시 중단 횟수에 대한 설명은 이 항목의 뒷부분에 나오는 "스레드 중지 및 재개"를 참조하십시오.  
  
-   합니다 **프로세스 이름** 각 스레드가 속하는 프로세스를 포함 하는 열 (일반적으로 숨김). 여러 프로세스를 디버깅 하는 경우이 열은 유용할 수 있습니다.  
  
### <a name="to-display-the-threads-window-in-break-mode-or-run-mode"></a>중단 모드나 실행 모드에서 스레드 창을 표시하려면  
  
-   디버깅 하는 동안 선택 된 **디버그** 메뉴에서 **Windows**를 클릭 하 고 **스레드**.  
  
### <a name="to-display-or-hide-a-column"></a>열을 표시하거나 숨기려면  
  
-   맨 위에 있는 도구 모음에서의 **스레드** 창에서 클릭 **열**, 한 다음 선택 하거나 열을 표시 하거나 숨기려면 이름의 선택을 취소 합니다.    

## <a name="display-flagged-threads"></a>플래그가 지정 된 스레드만 표시  
 아이콘 표시 하 여 특별 한 주의가 하려는 스레드에 플래그를 수는 **스레드** 창입니다. 자세한 내용은 [방법: 플래그 및 스레드의 플래그 해제](../debugger/how-to-flag-and-unflag-threads.md)합니다. 에 **스레드** 창 모든 스레드 또는 플래그가 지정 된 스레드만 표시 하도록 선택할 수 있습니다.  
  
#### <a name="to-display-only-flagged-threads"></a>플래그가 지정된 스레드만 표시하려면  
  
-   선택 된 **스레드 스레드만 표시** 맨 위에 있는 단추를 **스레드** 창입니다. (흐리게 표시 되 면 해야 먼저 일부 스레드에 플래그를 설정 합니다.) 

## <a name="freeze-and-thaw-threads"></a>중지 하 고 스레드 재개  
 스레드를 중지하면 리소스를 사용할 수 있어도 스레드 실행이 시작되지 않습니다.  
  
 네이티브 코드에서 일시 중단 하거나 Windows 함수를 호출 하 여 스레드를 다시 시작할 수 있습니다 `SuspendThread` 하 고 `ResumeThread` 또는 MFC 함수 [CWinThread::SuspendThread](/cpp/mfc/reference/CWinThread-class#suspendthread) 고 [cwinthread:: Resumethread](/cpp/mfc/reference/CWinThread-class#resumethread). 호출 하는 경우 `SuspendThread` 또는 `ResumeThread`, 변경한 합니다 *일시 중단 횟수*에 표시 되는 **스레드** 창입니다. 그러나 네이티브 스레드를 중지하거나 재개하는 경우에는 일시 중단된 횟수를 변경하지 않습니다. 네이티브 코드에서는 스레드가 재개되고 일시 중단된 횟수가 0인 경우 이외에는 스레드를 실행할 수 없습니다.  
...
1.  맨 위에 있는 도구 모음에는 **스레드** 창 열의 맨 위에 있는 단추를 클릭 합니다.  
...
-   에 **스레드** 창 도구 모음에서 클릭 합니다 **그룹화** 목록 스레드를 그룹화 하려는 조건을 클릭 합니다.  
  
#### <a name="to-sort-threads-within-groups"></a>그룹 내에서 스레드를 정렬하려면  
  
1.  맨 위에 있는 도구 모음에서 합니다 **스레드** 창 클릭 합니다 **그룹화** 목록 스레드를 그룹화 하려는 조건을 클릭 합니다.  
  
2.  에 **스레드** 창 열의 맨 위에 있는 단추를 클릭 합니다.  
  
     이제 해당 열의 값으로 스레드가 정렬됩니다.  
  
#### <a name="to-expand-or-collapse-all-groups"></a>모든 그룹을 확장하거나 축소하려면  
  
-   맨 위에 있는 도구 모음에서의 **스레드** 창에서 클릭 **그룹 확장** 또는 **그룹 축소**합니다.  
  
## <a name="search-for-specific-threads"></a>특정 스레드 검색  
 [!INCLUDE[vs_dev11_long](../data-tools/includes/vs_dev11_long_md.md)]에서 지정된 문자열과 일치하는 스레드를 검색할 수 있습니다. 스레드에 대해 검색 하는 경우는 **스레드** 창 창의 모든 열에서 검색 문자열과 일치 하는 모든 스레드를 표시 합니다. 정보에서 호출 스택의 맨 위에 있는 나타나는 스레드 위치가 포함 된 **위치** 열입니다. 그러나 기본적으로 전체 호출 스택을 검색 되지는 않습니다.  
