...
ms.locfileid: "49933770"
---
# <a name="navigate-code-with-the-visual-studio-debugger"></a>Visual Studio 디버거로 코드 탐색
명령 및 바로 가기를 디버거에서 코드를 탐색 하는 데 익숙한을 가져와 빠르고 쉽게 찾고 앱 관련 문제를 해결 하 게 합니다. 디버거에서 코드를 탐색 하는 동안에 앱의 상태를 검사 하거나 해당 실행 흐름에 자세히 알아보려면 수 있습니다.  
...
## <a name="start-debugging"></a>디버깅 시작  
 ...
 디버거가 일시 중지 된 동안에 지역 창에서 변수 값 보기 또는 조사식 창에서 식을 계산 하는 등 대부분의 디버거 기능을 사용할 수 (라고도 *중단 모드*). 디버거가 일시 중지 하는 경우에 앱 상태 함수, 변수, 하는 동안 일시 중단 되 고 개체가 메모리에 남아 있습니다. 중단 모드에서 요소의 위치와 상태 위반이 나 버그를 확인할 수 있습니다. 일부 프로젝트 유형의 경우에 중단 모드에서 응용 프로그램을 조정할을 만들 수 있습니다. 이러한 기능을 보여 주는 비디오를 시청 하려면 [디버거를 사용 하 여 시작](https://www.youtube.com/watch?v=FtGCi5j30YU&list=PLReL099Y5nRfw6VNvzMkv0sabT2crbSpK&index=6)합니다.
...
-   **실행 하려면 클릭** 
    ...
    디버거에서 일시 중지 된 동안 코드의 지점으로를 실행 하려면 선택 합니다 **여기까지 실행** 녹색 화살표 아이콘 (아이콘이 표시 코드 줄을 가리킬 때). 이렇게 하면 임시 중단점을 설정할 필요가 없습니다.
...
>   -   편집하며 계속하기를 활성화한 경우 편집하며 계속하기에서 즉시 다시 매핑할 수 없는 종류의 편집을 수행하면 **다음 문 설정** 이 실패합니다. 예를 들어 catch 블록 내의 코드를 편집하면 이 문제가 발생합니다. 이 경우 지원 작업이 하지는 않는다는 오류 메시지가 표시 됩니다.  
> 
> [!NOTE]
...
## <a name="BKMK_Restrict_stepping_to_Just_My_Code"></a>한 단계씩 사용자 코드가 아닌 코드 실행  
