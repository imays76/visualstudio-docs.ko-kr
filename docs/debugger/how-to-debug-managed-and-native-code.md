...
* Visual Studio가 설치 되어 있어야 하며 **c + +를 사용한 데스크톱 개발** 워크 로드.
    ...
    아직 Visual Studio를 설치 하지 않은 경우 이동 합니다 [Visual Studio 다운로드](https://visualstudio.microsoft.com/downloads/?utm_medium=microsoft&utm_source=docs.microsoft.com&utm_campaign=button+cta&utm_content=download+vs2017) 체험 용으로 설치 하는 페이지입니다.

    워크로드를 설치해야 하지만 이미 Visual Studio가 있는 경우 **새 프로젝트** 대화 상자의 왼쪽 창에서 **Open Visual Studio 설치 관리자** 링크를 클릭합니다. Visual Studio 설치 관리자가 시작됩니다. **C++를 사용한 데스크톱 개발** 워크로드를 선택한 다음, **수정** 단추를 선택합니다.
...
1. 에 **새 프로젝트** 대화 상자에서 **Visual c + +**, **기타** 한 다음 가운데 창의 설치 된 템플릿 섹션에서 선택 **빈 프로젝트** .

1. 에 **이름을** 필드에 입력 **Mixed_Mode_Debugging** 클릭 **확인**합니다.

    Visual Studio에 오른쪽 창에서 솔루션 탐색기에 나타나는 빈 프로젝트를 만듭니다.

1. 솔루션 탐색기에서 마우스 오른쪽 단추로 클릭 합니다 **소스 파일** 노드 c + + 프로젝트를 선택한 후 **추가** > **새 항목**를 선택한 후 **c + + 파일 (.cpp)** 합니다. 파일 이름을 **Mixed_Mode.cpp**, 선택한 **추가**합니다.
...
1. 다음 코드를 복사 *Mixed_Mode.cpp*:
    ...
    ```
1. 솔루션 탐색기에서 마우스 오른쪽 단추로 클릭 합니다 **헤더 파일** 노드 c + +에서 프로젝트를 선택한 후 **추가** > **새 항목**를 선택한 후  **헤더 파일 (.h)** 합니다. 파일 이름을 **Mixed_Mode.h**, 선택한 **추가**합니다.
...
1. 디버그 도구 모음에서 선택 된 **디버그** 구성 하 고 **x86** 또는 **x64** 플랫폼으로 (항상 64 비트 모드로 실행 되는.NET Core를 선택 **x64**  플랫폼으로).

1. 솔루션 탐색기에서 프로젝트 노드를 마우스 오른쪽 단추로 (**Mixed_Mode_Debugging**)를 선택 하 고 **속성**합니다.

    > [!IMPORTANT]
    > C + +에 대 한 속성 구성을 플랫폼 당입니다. 따라서는 다른 (x86에 x64 하거나 그 반대 방향) 간에 전환 하면 새 구성의 속성을 설정 해야 합니다. (속성 페이지에서 확인 하거나 **x64** 하거나 **Win32** 페이지의 맨 위에 있는 플랫폼으로 설정 됩니다.)

1. 에 **속성** 페이지에서 **구성 속성** > **링커** > **고급**, 및 그런 다음 합니다 **진입점 없음** 드롭 다운 나열 되어 있는지 확인 합니다 **아니요** 을 선택 합니다. 설정을 변경 해야 할 경우 **No**, 선택한 **적용**합니다.
...
1. 선택한 프로젝트를 마우스 오른쪽 단추로 클릭 **빌드**합니다.
...
1. Visual Studio에서 선택 **파일** > **새로 만들기** > **프로젝트**합니다.
    ...
    > [!NOTE]
    > 새 솔루션을 만드는 대신 c + + 프로젝트를 솔루션에 새 관리 되는 프로젝트를 추가할 수도 있지만에서는 수행 하지 않습니다는 여기 디버깅 시나리오 더 큰 집합을 지원 하기 위해.
...
1. 새 코드에서 이전에 만든 (코드 주석을 참조) 하는 DLL에 대 한 경로에 파일 경로 업데이트 합니다. 바꿔야 합니다 *username* 자리 표시자입니다.
