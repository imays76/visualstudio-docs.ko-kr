---
title: IntelliTrace 단계 백을 사용 하 여 스냅숏 보기
...
1. 예외에서 보기 스냅숏에 대 한 옵션을 구성 하려는 경우 선택 **IntelliTrace** > **고급** 에서 **옵션** 대화 상자.

    이러한 옵션은 Visual Studio 2017 Enterprise 15.7 버전부터 사용할 수 있습니다.

    ![예외에서 스냅숏에 대 한 동작을 구성 합니다.](../debugger/media/intellitrace-enable-snapshots-on-exceptions.png)

    이벤트 및 스냅숏을 사용 하면, 예외에 대 한 스냅숏을 기본적으로 사용 됩니다. 선택 해제 하 여 예외에 대 한 스냅숏을 사용 하지 않도록 설정할 수 **예외 이벤트에 대 한 스냅숏을 수집**합니다. 이 기능을 사용 하는 경우 처리 되지 않은 예외에 대 한 스냅숏이 만들어집니다. 처리 되는 예외에 대 한 스냅숏을 만드는 예외가 throw 되는 경우에 한 이전에 발생 한 예외를 다시 throw 되지 않은 경우. 드롭 다운 목록에서 값을 선택 하 여 예외에 대 한 최대 스냅숏 수를 설정할 수 있습니다. 최대 응용 프로그램 (예: 응용 프로그램 중단점을 적중 하는 경우) 중단 모드를 시작 하는 각 시간에 대해 적용 됩니다.

    > [!NOTE]
    > 스냅숏을은 IntelliTrace에서 기록 하는 예외 이벤트에 대해서만 수행 됩니다. 선택 하 여 IntelliTrace에서 기록 이벤트를 지정할 수 **도구** > **옵션** > **IntelliTrace 이벤트**합니다.

1. 프로젝트에서 하나 이상의 중단점을 설정 하 고 디버깅을 시작 (키를 눌러 **F5**), 코드를 단계별로 실행 하 여 디버깅을 시작 하거나 (**F10** 또는 **F11**).

    IntelliTrace는 각 단계 디버거, 중단점 이벤트 및 처리 되지 않은 예외 이벤트에 대 한 응용 프로그램의 프로세스의 스냅숏을 만듭니다. 이러한 이벤트에 기록 됩니다는 **이벤트** 탭에 **진단 도구** 다른 IntelliTrace 이벤트와 함께 창. 이 창을 열려면 선택 **디버그** > **Windows** > **진단 도구 표시**합니다.
