...
> [!NOTE]
> 워크플로 디자이너 사용자 인터페이스는 가상화 합니다. 활동 디자이너는 필요한 경우에만 렌더링됩니다. 디자이너 화면에 표시된 적이 없는 워크플로 부분이 있다면 그 부분은 개요 맵에서 흰색으로 나타납니다. 개요 맵 주위를 스크롤하면 워크플로가 완전히 그려집니다.
...
내 Visual Studio 2010 워크플로 디자이너에서 사용 되는 글꼴 환경 글꼴에 의해 제어 됩니다. 워크플로 디자이너에 표시 된 색에 대 한 운영 체제 테마에 고대비 색 구성표를 사용 하는 경우 변경 합니다. 워크플로 디자이너에서 변경 내용이 적용 되기 전에에 글꼴 또는 색상 설정을 변경한 후 Visual Studio 2010를 다시 시작 해야 합니다.