...
| DV1003: **허용 되지 않는 네임 스페이스 이름** | 이 문제는 "Namespace 이름은 허용 되지 않는" 속성에는이 코드 요소가 정의 된 네임 스페이스를 포함 하는 레이어와 연결 된 코드 요소에서 보고 됩니다. 이 명명 제약 조건을 위반 합니다. "네임 스페이스 이름을 허용 안 함" 속성은 네임 스페이스는 코드에서이 레이어와 연결 된 요소 정의 해서는 안 세미콜론으로 구분 된 목록으로 정의 note 합니다. |
