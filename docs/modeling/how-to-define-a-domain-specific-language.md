...
### <a name="test-the-solution"></a>솔루션 테스트
 ...
 솔루션을 테스트하려면 F5 키나 Ctrl+F5를 누릅니다. Visual Studio의 새 인스턴스를 실험적 모드에서 엽니다.
...
2.  **빌드하고 DSL을 실행 합니다.** Visual Studio의 새 인스턴스를 실험적 모드에서 실행 하려면 F5 키나 ctrl+f5를 누릅니다. Visual Studio의 실험적 인스턴스에서 열거나 DSL의 파일 이름 확장명을 가진 파일을 만듭니다.

3.  **탐색기를 엽니다.** At 다이어그램의 측면은 일반적으로 이름으로 지정 된 언어 탐색기 창이 *YourLanguage* 탐색기. 이 창이 표시되지 않으면 솔루션 탐색기 아래쪽의 탭에 있을 수 있습니다. 이 찾을 수 없는 경우는 **보기** 메뉴에서 **기타 Windows**를 클릭 하 고 *YourLanguage* **탐색기**합니다.
...
|어셈블리 이름을 변경하지 않았는데 DSL의 도구 상자가 표시되지 않습니다.<br /><br /> 또는 확장을 로드하지 못했음을 보고하는 메시지 상자가 표시됩니다.|실험적 인스턴스를 다시 설정하고 솔루션을 다시 빌드합니다.<br /><br /> 1.  Windows 시작 메뉴에서 **모든 프로그램**, 확장 [!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)], 한 다음 **도구**, 클릭 하 고 **Microsoft Visual Studio 실험적 인스턴스 재설정**.<br />2.  Visual studio**빌드합니다** 메뉴에서 클릭 **솔루션 다시 빌드**합니다.|
