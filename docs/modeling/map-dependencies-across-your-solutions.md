...
코드 맵을 만들어서 코드 전체에서 종속성을 시각화할 수 있습니다. 코드 맵을 어떻게 여러 코드가 서로 맞는지 파일과 코드 줄을 읽지 않더라도 확인할 수 있습니다.

![Visual Studio에서 코드 맵 사용 하 여 종속성 보기](../modeling/media/codemapsmainintro.png)
...
> [!NOTE]
> Visual Studio Professional을 사용 하는 다른 사용자와 Visual Studio Enterprise에서 만든 맵을 공유 하기 전에 (예: 숨겨진된 항목, 확장 된 그룹 및 그룹 간 링크) 지도에 있는 모든 항목이 표시 되는지 확인 합니다.

이러한 언어의 코드에 대 한 종속성을 매핑할 수 있습니다.
...
> [!NOTE]
> C# 또는 Visual Basic이 아닌 다른 프로젝트에 코드 맵을 시작 하거나 기존 코드 맵에 항목을 추가 하는 더 적은 옵션이 있습니다. 예를 들어 C++ 프로젝트의 텍스트 편집기에서는 개체를 마우스 오른쪽 단추로 클릭하여 코드 맵에 추가할 수 없습니다. 끌어 하 고 개별 코드 요소 또는 파일에서 삭제할 수 있지만 **솔루션 탐색기**를 **클래스 뷰**, 및 **개체 브라우저**합니다.
...
1. 아래로 스크롤하여 합니다 **코드 도구** 선택한 섹션 **코드 맵** 및 **실시간 종속성 유효성 검사**합니다.

   ![Visual Studio 설치 관리자의 코드 맵 및 실시간 종속성 유효성 검사 구성 요소](media/modeling-components.png)

1. **수정**을 선택합니다.

   합니다 **코드 맵** 하 고 **실시간 종속성 유효성 검사** 구성 요소 설치를 시작 합니다. Visual Studio를 닫을 하 라는 메시지가 표시 될 수 있습니다.

## <a name="add-a-code-map"></a>코드 맵을 추가합니다

빈 코드 맵을 만들고 있고 끌어 항목에 어셈블리 참조, 파일 및 폴더를 포함 하 여 수도 있고, 솔루션의 일부나 모두에 대 한 코드 맵을 생성할 수 있습니다.

빈 코드 맵을 추가 합니다.

1. **솔루션 탐색기**에서 최상위 솔루션 노드의 바로 가기 메뉴를 엽니다. 선택할 **추가할** > **새 항목**합니다.

2. 에 **새 항목 추가** 대화 상자 아래에 있는 **설치 됨**, 선택는 **일반** 범주.

3. 선택 된 **Directed Graph Document(.dgml)** 템플릿을 선택한 후 **추가**합니다.

   > [!TIP]
   > 표시 되지 않는 경우 템플릿 목록의 맨 아래로 스크롤하여 이므로이 템플릿은 사전순으로 나타나지 않을 수 있습니다.

   빈 맵을 만들려면 솔루션에 나타납니다 **솔루션 항목** 폴더입니다.

마찬가지로, 새 코드 맵 파일을 선택 하 여 솔루션에 추가 하지 않고 만들 수 있습니다 **아키텍처** > **새 코드 맵에** 하거나 **파일**  >  **새** > **파일**합니다.

## <a name="generate-a-code-map-for-your-solution"></a>솔루션용 코드 맵을 생성합니다

솔루션의 모든 종속성을 보려면:

1. 메뉴 모음에서 선택 **아키텍처** > **솔루션용 코드 맵 생성**합니다. 마지막으로 빌드한 후 코드가 변경 되지 않은, 경우 선택할 수 있습니다 **아키텍처** > **빌드하지 않고 솔루션용 코드 맵 생성** 대신 합니다.

   ![코드 맵 생성 명령](../modeling/media/codemapsarchitecturemenu.png)

   지도 최상위 수준 어셈블리와 어셈블리 간의 집계 된 링크를 보여 주는 생성 됩니다. 집계 링크의 범위가 넓을수록 해당 링크로 표시되는 종속성의 수도 많습니다.
...
4. 그러나 이 방법은 큰 솔루션에는 유용하지 않을 수 있습니다. 실제로 복잡한 솔루션의 경우 메모리 제한으로 인해 모든 그룹을 확장하지 못할 수 있습니다. 대신에 개별 노드의 내부를 확인하려면 해당 노드를 확장합니다. 마우스 포인터를 노드 맨 위로 이동하고 펼침 단추(아래쪽 화살표)가 나타나면 단추를 클릭합니다.
   ...
   > [!TIP]
   > 코드로 작업 하는 방법에 대 한 자세한 내용은 마우스, 키보드 및 터치 참조를 사용 하 여 매핑합니다 [찾아보기 및 다시 정렬 코드 맵](../modeling/browse-and-rearrange-code-maps.md)합니다.
...
6. 항목 간 관계를 확인하려면 맵에서 관계를 선택합니다. **범례** 창에 표시된 대로 링크 색은 관계 형식을 나타냅니다.
   ...
   > [!TIP]
   > 녹색 링크가 표시된다고 해서 단순히 상속 관계만 있는 것은 아닙니다. 메서드 호출도 있을 수 있지만 해당 호출은 상속 관계에 의해 숨겨집니다. 특정 유형의 링크를 보려면 확인란을 사용 합니다 **필터** 창 원하지 않는 형식을 숨깁니다.
...
9. 맵의 특정 부분에 포커스를에 관심이 없는 항목을 제거 하려면 계속 수 있습니다. 예를 들어 클래스 및 멤버 뷰를 분석하려면 **필터** 창에서 모든 네임스페이스 노드를 필터링하면 됩니다.
...
- 하 여 코드의 잠재적 문제 찾기 [분석기 실행](../modeling/find-potential-problems-using-code-map-analyzers.md)

## <a name="view-specific-dependencies-in-a-code-map"></a>코드 맵에서 특정 종속성 보기

보류 중인 변경 내용 사용 하 여 일부 파일에서 수행 하는 코드 검토를 있다고 가정 합니다. 변경 내용의 종속성을 확인하려면 해당 파일에서 코드 맵을 만듭니다.

   ![코드 맵에 특성 종속성 표시](../modeling/media/codemapsspecificdependenciesintro.png)

1. **솔루션 탐색기**, 프로젝트, 어셈블리 참조, 폴더, 파일, 형식 또는 매핑하려는 멤버를 선택 합니다.

   ![매핑할 항목 선택](../modeling/media/codemapsselectinsolutionexplorer.png)

1. 에 **솔루션 탐색기** 도구 모음 선택 **코드 맵에 표시** ![만들 새 그래프에서 선택한 노드 단추](../modeling/media/createnewgraphfromselectedbutton.gif)합니다. 또는 하나 또는 항목 그룹에 대 한 바로 가기 메뉴를 열고 선택 **코드 맵에 표시**합니다.

   항목을 끌 수도 있습니다 **솔루션 탐색기**를 **클래스 뷰**, 또는 **개체 브라우저**에 [새](#add-a-code-map) 또는 기존 코드를 매핑합니다. 항목의 부모 계층 구조를 포함 하려면 키를 누른 합니다 **Ctrl** 항목을 끌거나 사용 하 여 키를 **부모 포함** 기본 동작을 지정 하려면 코드 맵 도구 모음에서 단추입니다. 와 같이 Visual Studio 외부에서 어셈블리 파일을 끌 수도 있습니다 **Windows 탐색기**합니다.

   > [!NOTE]
   > Windows Phone 또는 Microsoft Store 같은 여러 앱 간에 공유 되는 프로젝트에서 항목을 추가 하면 해당 항목 현재 활성화 된 앱 프로젝트와 함께 맵에 나타납니다. 다른 앱 프로젝트에 대한 컨텍스트를 변경하고 공유 프로젝트의 항목을 추가하면 해당 항목이 새로 활성화된 앱 프로젝트와 함께 나타납니다. 맵의 항목에 수행하는 작업은 동일한 컨텍스트를 공유하는 항목에만 적용됩니다.
...
4. 항목을 탐색하려면 해당 항목을 확장합니다. 항목을 확장하려면 항목 위로 마우스 포인터를 이동한 다음 펼침 단추(아래쪽 화살표) 아이콘이 나타나면 클릭합니다.
   ...
   ![코드 맵에서 노드를 확장 합니다.](../modeling/media/dependencygraph_containment.png)

   선택 항목을 모두를 확장 하려면 사용 하 여 **Ctrl**+**A**, 한 다음 맵에 대 한 바로 가기 메뉴를 열고 선택 **그룹**  >   **확장**합니다. 그러나 이 옵션은 모든 그룹을 확장할 때 사용할 수 없는 맵 또는 메모리 문제가 발생하는 경우 사용할 수 없습니다.
...
7. 맵에 관계가 표시됩니다. 이 예에서 맵을 호출할 메서드를 보여 줍니다.는 `Find` 메서드 및 솔루션 또는 외부에서 위치입니다.
