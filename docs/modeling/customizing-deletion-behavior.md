...
-   [Delete Closure 재정의](#closure) -이 방법을 사용 하 여 인접 요소도 삭제 삭제는 발생할 수 있습니다.

-   [OnDeleting 및 OnDeleted 사용](#ondeleting) -이러한 방법을 사용 하 여 응답 값을 내부 또는 외부 저장소를 업데이트 하는 등 다른 작업에 포함 될 수 있습니다 위치 합니다.
...
2.  **삭제** store 이벤트는 트랜잭션이 끝날 때 호출 되 고 실행 취소 또는 다시 실행 한 후 호출 됩니다. 개체 파일, 데이터베이스 항목 또는 Visual Studio의 다른 개체와 같은 store 외부에 있는 삭제 전파를 따라서 사용 수 있습니다.
    ...
    > [!WARNING]
    >  요소가 삭제된 경우 해당 도메인 속성 값에는 액세스할 수 있지만 관계 링크를 탐색할 수는 없습니다. 그러나 관계에 대해 Deleted 이벤트를 설정한 경우 역할 수행자였던 두 요소에도 액세스할 수 있습니다. 따라서 모델 요소 삭제에 응답 하지만 연결 된 요소에 액세스 하려는 경우 모델 요소의 도메인 클래스가 아닌 관계에 대해 delete 이벤트를 설정 합니다.
