...
2.  로컬 규칙 집합에 Visual Studio에서 프로젝트를 덜 제한적인 **동작** 프로젝트 규칙 집합으로 설정 된 규칙을 예를 들어 외 설정 **동작**=**오류** 서버에 해당 **작업** 로 설정 **경고** 하거나 **None** Visual Studio에서 실행 되 고 설정 규칙에).

3.  규칙 집합 Visual Studio에서 지정 된 모든 프로젝트에 대 한 코드 분석 체크 인 정책에 지정 된 규칙에 지정 된 규칙이 없습니다.
