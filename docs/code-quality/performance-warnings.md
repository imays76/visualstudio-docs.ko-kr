...
| [CA1802: 리터럴을 사용 하십시오.](../code-quality/ca1802-use-literals-where-appropriate.md) | 정적 및 읽기 전용 필드가 선언 되었습니다 (Shared 및 ReadOnly에서 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]), 컴파일 시간에 계산할 수 있는 값으로 초기화 됩니다. 컴파일 시간에 대상된 필드에 할당 되는 값을 계산할 이기 때문에 const 선언을 변경 (에서 Const [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]) 필드 값을 런타임 대신 컴파일 시간에 계산 되도록 합니다. |
