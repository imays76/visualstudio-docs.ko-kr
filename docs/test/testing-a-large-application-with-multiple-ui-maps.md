...
-   응용 프로그램의 다양한 지점에서 액세스되는 독립 컨트롤 집합(예: 여러 작업 페이지가 있는 마법사) 마법사의 각 페이지가 특히 더 복잡한 경우 각 페이지에 대한 개별 UI 맵을 만들 수 있습니다.
