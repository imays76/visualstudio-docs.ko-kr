...
|**애플리케이션 도메인에서 단위 테스트 실행**|부하 테스트에 단위 테스트가 포함되어 있는 경우 각 단위 테스트 어셈블리를 별도의 응용 프로그램 도메인에서 실행할 것인지를 결정하는 부울 값입니다. 기본 설정은 True입니다.<br /><br /> 단위 테스트에서 개별 응용 프로그램 도메인 또는 app.config 파일이 올바르게 작동할 필요가 없는 경우에는 이 속성 값을 `False`로 설정하여 단위 테스트의 실행 속도를 높일 수 있습니다.|
