...
- [Project 요소](../msbuild/project-element-msbuild.md)는 새 `SDK` 특성을 포함합니다. 이제 `Xmlns` 특성도 선택 사항입니다. `SDK` 특성에 대한 자세한 내용은 [방법: MSBuild Project SDK 사용](../msbuild/how-to-use-project-sdk.md), [패키지, 메타데이터, 프레임워크](/dotnet/core/packages) 및 [.NET 코어용 csproj 형식에 대한 추가 사항](/dotnet/core/tools/csproj)을 참조하세요.
