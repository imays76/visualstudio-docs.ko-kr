...
|`S_FALSE`|스크립트 실행 되지 않는 경우 프로 파일링을 시작 했습니다.|  
|`ACTIVPROF_E_PROFILER_ABSENT`|프로 파일링 사용 되지 않습니다. 콜백이 없는 설정 되었습니다.|  
|`E_OUTOFMEMORY`|메모리 부족 상태로 인해 호출 스택을 얻을 수 없습니다.|  
  
## <a name="remarks"></a>설명  
 호출 `IActiveScriptProfilerControl2::CompleteProfilerStart` 이미 호출 스택에 있는 함수에 대 한 이벤트 전송 되는지 확인 합니다. 이 메서드는 현재 탭에 있는 모든 스크립팅 엔진에서 시작을 프로 파일링 한 후 호출할 수 있습니다. 모든 스크립팅 엔진에 대 한 메서드를 호출할 수 있습니다.  
