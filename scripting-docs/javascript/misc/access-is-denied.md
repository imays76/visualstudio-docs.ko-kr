...
스크립트가 현재 페이지의 호스트가 아닌 다른 소스에서 데이터에 액세스하려고 했습니다. Internet Explorer 및 다른 브라우저가 따르는 동일 원본 정책에서는 스크립트가 현재 페이지 URL의 체계, 호스트 및 포트와 동일한 소스의 데이터만 액세스할 수 있습니다.  
 ...
 예를 들어 현재 페이지가 https://employees.mycompany.com 이면 다음 URL의 데이터에 액세스할 수 없습니다.  
  
-   [http://data.contoso.com](http://data.contoso.com) (HTTPS가 아니라 HTTP를 사용하므로)  
  
-   [https://somedatasource.com](https://somedatasource.com) (다른 도메인에 있으므로)  
