...
ms.locfileid: "24637103"
---
# <a name="error-object-javascript"></a>Error 개체(JavaScript)
오류 정보가 들어 있습니다.  
...
## <a name="parameters"></a>매개 변수  
 `errorObj`  
 필수 요소. `Error` 개체가 할당되는 변수 이름입니다. `throw` 문을 사용하여 오류를 만들면 변수 할당이 생략됩니다.  
  
 `number`  
 선택 사항입니다. 오류에 할당된 숫자 값입니다. 생략하면 0이 할당됩니다.  
  
 `description`  
 선택 사항입니다. 오류를 설명하는 간단한 문자열입니다. 생략하면 빈 문자열이 할당됩니다.  
  
## <a name="remarks"></a>설명  
 런타임 오류가 발생할 때마다 `Error` 개체의 인스턴스가 생성되어 오류를 설명합니다. 이 인스턴스에는 오류 설명이 포함되어 있는 두 가지 내장 속성(`description` 속성) 및 오류 번호(`number` 속성)가 있습니다. 자세한 내용은 참조 [JavaScript 런타임 오류](../../javascript/reference/javascript-run-time-errors.md)합니다.  
 ...
 생성 된 지역 변수는 일반적으로 **try … catch** 문에서 참조는 암시적으로 생성 `Error` 개체입니다. 그 결과 사용자가 선택한 방법으로 오류 번호 및 설명을 사용할 수 있습니다.  
